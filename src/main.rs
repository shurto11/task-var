...
//! - 全画面クライアントのシーン(`SWIPE_SCENES`: fbhalf)の間は
...
//! - ssbrowse はスワイプ表示モードにしない。tmux のステータス行と同じく端末を
//!   縮めたままにするので、ssbrowse のペイン(= ブラウザの描画領域)がバーの
//!   上で完結し、バーは常時表示のままかぶらない。
...
/// 上スワイプでだけ一時表示する。
///
/// ssbrowse は含めない: tmux ペインを追って描画するクライアントなので、端末を
/// 縮めておけばブラウザの描画領域がバーの上で完結し、tmux のステータス行と
/// 同じようにバーを常時表示したままかぶらずに済む。
const SWIPE_SCENES: &[&str] = &["fbhalf"];
...
fn daemon() -> Result<()> {
    ...
    // tmux ペインを追う全画面クライアント(ssbrowse)は、これだけでバーを避ける。
