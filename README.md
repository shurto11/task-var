...
ChromeBook(ChromeOS)のシェルフのように使うために作った。画面下端に常駐する
帯をタップするだけでアプリの起動と切り替えが済み、キーボードを触らずに
端末機として使える。X もウィンドウマネージャも使わず、`/dev/fb0` へ直接描く。

ChromeOS でいうシェルフとの対応は次のとおり。

| ChromeOS | task-var |
|----------|----------|
| シェルフのアプリアイコン | アイコン列。タップで tmux セッションを起動・切り替え |
| アイコン下のインジケーター | 白円の下の横線。起動中・表示中を色と長さで表す |
| ステータストレイのメディア通知 | 再生情報パネル。曲の表示と再生操作 |
| ウィンドウ一覧 | Claude セッション枠。動いている Claude Code へ直接飛べる |
